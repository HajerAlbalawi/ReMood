...
function getOrigin(_req: Request): string {
  ...
  if (configuredOrigins[0]) return configuredOrigins[0].origin;
...
function getSafeReturnTo(value: unknown): '/' | '/history' | '/stats' {
  if (
    typeof value !== 'string' ||
    !value.startsWith('/') ||
    value.startsWith('//') ||
  ...
  try {
    const parsed = new URL(value, 'https://remood.invalid');
    if (parsed.origin !== 'https://remood.invalid') return '/';
    if (parsed.pathname === '/history') return '/history';
    if (parsed.pathname === '/stats') return '/stats';
    return '/';
  } catch {
    return '/';
  }
...
router.get('/callback', async (req: Request, res: Response) => {
  ...
  if (returnTo === '/history') res.redirect('/history');
  else if (returnTo === '/stats') res.redirect('/stats');
  else res.redirect('/');
});
...
router.get('/logout', async (req: Request, res: Response) => {
  ...
  res.status(302).setHeader('Location', endSessionUrl.href).end();
});
